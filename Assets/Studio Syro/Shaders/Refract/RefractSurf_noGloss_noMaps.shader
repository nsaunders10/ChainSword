...
Shader "Refractive/RefractSurf_noGloss_noMaps" {
	Properties {
		...
		SubShader{
			...
			void surf (Input IN, inout SurfaceOutputStandard o) {
				...
				float2 dispVec = mul(fragmentViewBasis, float4(IN.worldNormal, 0.0)).xy;
				...
				float fresnel = pow( 1-max(dot(IN.worldNormal, camZ),0.0), _FresnelPower ) * _FresnelIntensity;
				dispVec *= fresnel; 
